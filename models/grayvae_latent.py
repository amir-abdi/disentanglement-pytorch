...
#        self.classification = nn.Linear(self.z_dim, 1, bias=False).to(self.device)
        ...
        self.generation = nn.Linear(1, 2*self.z_dim)  #nn.Sequential(nn.Linear(1, 10), nn.ReLU(), nn.Linear(10, 2*self.z_dim) )
        
        self.classification_epoch = args.classification_epoch
        self.reduce_rec = args.reduce_recon

        self.class_G_all = optim.Adam([*self.model.parameters(), *self.classification.parameters(), *self.generation.parameters()],
